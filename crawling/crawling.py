...
producer = KafkaProducer(acks = 1, compression_type = None, bootstrap_servers = ['localhost:9092'])
            ...
            for ul in uls :
                ...
                for link in links : 
                    ...
                    if link.get("href") == latest_data[str(id)]['latest'] : 
                        ...
                        with open("C:/Users/SSAFY/Desktop/latest.json","w", encoding="UTF8") as update :
...
cdic = {}
# EC2에 올리고 경로 수정 필요
with open("C:/Users/SSAFY/Desktop/config.json","r", encoding="UTF8") as file :
    data = json.load(file)
...
# EC2에 올리고 경로 수정 필요
with open("C:/Users/SSAFY/Desktop/latest.json","r", encoding="UTF8") as latest :
...
for i in range(7,1,-1):
    ...
    producer.send("NEWS", value = json_data.encode('utf-8'))
    producer.flush()
    # EC2에 올리면 아래 코드는 주석처리
    with open(f'C:/Users/SSAFY/Desktop/output/{today}.json','w',encoding='utf-8',) as make_file:
